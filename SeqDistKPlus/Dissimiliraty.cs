    ...
    class Dissimiliraty
    {
        ...
        private void CalHao(SequenceData seqX, SequenceData seqY, Result result, int k, int m = 0)
        {
            try
            {
                ...
                IList<int> listKtupleX = seqX.GetKtupleData(k);
                IList<double> listPosX = seqX.GetMarkovData(k, k - 2);
                int totalX = seqX.GetTotal(k);

                IList<int> listKtupleY = seqY.GetKtupleData(k);
                IList<double> listPosY = seqY.GetMarkovData(k, k - 2);
        ...
        private void CalEu(SequenceData seqX, SequenceData seqY, Result result, int k, int m = 0)
        {
            ...
            SuperList<int> listKtupleX = seqX.GetKtupleData(k);
            int totalX = seqX.GetTotal(k);

            SuperList<int> listKtupleY = seqY.GetKtupleData(k);
            int totalY = seqY.GetTotal(k);

            //Eu
            for (long i = 0; i < listKtupleX.LongCount; i++)
            {
                //public
                cXi = listKtupleX[i];
                cYi = listKtupleY[i];
        ...
        private void CalD2S(SequenceData seqX, SequenceData seqY, Result result, int k, int m)
        {
            ...
            //public
            IList<int> listKtupleX = seqX.GetKtupleData(k);
            IList<double> listPosX = seqX.GetMarkovData(k, m);
            double totalX = seqX.GetTotal(k);

            IList<int> listKtupleY = seqY.GetKtupleData(k);
            IList<double> listPosY = seqY.GetMarkovData(k, m);
